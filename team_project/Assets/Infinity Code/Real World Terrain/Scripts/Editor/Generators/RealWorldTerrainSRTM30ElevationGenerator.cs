...
namespace InfinityCode.RealWorldTerrain.Generators
{
    public class RealWorldTerrainSRTM30ElevationGenerator : RealWorldTerrainElevationGenerator
    {
        ...
        private static List<RealWorldTerrainDownloadItemWebClient> downloaderItems;
        ...
        public override void UnzipHeightmap()
        {
            ...
            ZipArchive zip = ZipFile.OpenRead(arcFilename);
            
            foreach (ZipArchiveEntry entry in zip.Entries)
            {
                if (entry.Name != localFN) continue;
                
                entry.ExtractToFile(heightmapFilename);
                zip.Dispose();
                unziped = true;
                return;
            }
