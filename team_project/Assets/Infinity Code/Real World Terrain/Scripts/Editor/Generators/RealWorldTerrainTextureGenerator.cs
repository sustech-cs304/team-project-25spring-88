...
namespace InfinityCode.RealWorldTerrain.Generators
{
    public class RealWorldTerrainTextureGenerator
    {
        ...
        private RealWorldTerrainTextureGenerator(int tileX, int tileY, int zoom)
        {
            ...
            double cx, cy;
            RealWorldTerrainGeo.TileToLatLong(tileX, tileY, zoom, out cx, out cy);
            RealWorldTerrainGeo.LatLongToMercat(cx, cy, out leftMercator, out topMercator);
            RealWorldTerrainGeo.TileToLatLong(tileX + 1, tileY + 1, zoom, out cx, out cy);
            RealWorldTerrainGeo.LatLongToMercat(cx, cy, out rightMercator, out bottomMercator);
            ...
            if (!File.Exists(path))
            {
                new RealWorldTerrainDownloadItemWebClient(url)
                {
                    filename = path,
        ...
        public static void Init()
        {
            ...
            {
                double bx, by, ex, ey;
                RealWorldTerrainGeo.LatLongToTile(prefs.leftLongitude, prefs.topLatitude, z, out bx, out by);
                RealWorldTerrainGeo.LatLongToTile(prefs.rightLongitude, prefs.bottomLatitude, z, out ex, out ey);
        ...
        private void Load()
        {
            ...
            {
                RealWorldTerrainFileSystem.SafeDeleteFile(path);
                return;
            }
        ...
        private string ReplaceToken(Match match)
        {
            ...
            if (v == "quad") return RealWorldTerrainGeo.TileToQuadKey(tileX, tileY, zoom);
            return v;
        }
