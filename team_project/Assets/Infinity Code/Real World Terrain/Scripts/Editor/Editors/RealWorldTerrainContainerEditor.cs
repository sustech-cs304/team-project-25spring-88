...
namespace InfinityCode.RealWorldTerrain.Editors
{
    ...
    public class RealWorldTerrainContainerEditor : Editor
    {
        ...
        {
            RealWorldTerrainContainer[] containers = RealWorldTerrainUtils.FindObjectsOfType<RealWorldTerrainContainer>();
        ...
        private void DrawItemSize(out float sizeX, out float sizeY)
        {
            ...
            if (container.prefs.sizeType == 0 || container.prefs.sizeType == 2)
            {
                ...
                sizeY = (float)(RealWorldTerrainGeo.EARTH_RADIUS * Math.Acos(scfY * sctY + ccfY * cctY));
            }
            else if (container.prefs.sizeType == 1)
            {
                sizeX = (float)Math.Abs(rx / 360 * RealWorldTerrainGeo.EQUATOR_LENGTH);
                sizeY = (float)Math.Abs((by - ty) / 360 * RealWorldTerrainGeo.EQUATOR_LENGTH);
            }
        ...
        private void DrawMinAndMaxElevations()
        {
            if (container == null || container.terrains == null || container.terrains.Length == 0) return;
            EditorGUILayout.LabelField($"Min Elevation: {container.minElevation}");
            EditorGUILayout.LabelField($"Max Elevation: {container.maxElevation}");
            EditorGUILayout.Space();
        }
        ...
        private void SplitTerrains()
        {
            foreach (RealWorldTerrainItem terrainItem in container.terrains)
            {
                ...
                c.size = terrainItem.size;
                c.scale = terrainItem.scale;
                c.minElevation = terrainItem.minElevation;
                c.maxElevation = terrainItem.maxElevation;
